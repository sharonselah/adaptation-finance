...
(function () {
  ...
  function legend(s, x, y, items, opts) {
    ...
    if (opts.column) {
      items.forEach(([c, l]) => {
        el('rect', { x: x, y: y - box + 3, width: box, height: box, rx: 2, fill: c }, s);
        txt(s, x + box + 9, y - 3, l, { size: size, fill: DARK });
        y += box + 8;
      });
      return y;
    }
    let lx = x;
    items.forEach(([c, l]) => {
      el('rect', { x: lx, y: y - box + 3, width: box, height: box, rx: 2, fill: c }, s);
      txt(s, lx + box + 8, y - 3, l, { size: size, fill: DARK });
  ...
  function f02(host, D) {
    ...
    series.forEach(([k, c, w, dash], si) => {
      ...
      d[k].forEach((v, i) => { if (v !== null) el('circle', { cx: X(yrs[i]), cy: Y(v), r: w > 3 ? 4.6 : 3.4, fill: c }, s); });
      ...
      txt(s, R + 16, ly, Math.round(d[k][li]), { base: 'middle', size: 12.5, weight: 700, fill: c });
      txt(s, R + 52, ly, k, { base: 'middle', size: 12, fill: DARK });
    });
  ...
  function f03(host, D) {
    ...
    c.periods.forEach((p, i) => {
      ...
      cats.forEach(([k, , col], j) => {
        ...
        el('rect', { x: x, y: Y(c[k][i]), width: bw, height: B - Y(c[k][i]), fill: col, rx: 2 }, s);
    ...
    [['adaptation', TEAL], ['mitigation', MID]].forEach(([k, col]) => {
      el('path', { d: segs(m[k], m.years, MX, MY)[0], fill: 'none', stroke: col, 'stroke-width': 3.2, 'stroke-linejoin': 'round' }, s);
      m[k].forEach((v, i) => el('circle', { cx: MX(m.years[i]), cy: MY(v), r: 4, fill: col }, s));
    });
    m.years.forEach((y, i) => { if (i % 2 === 0) txt(s, MX(y), B + 22, String(y), { anchor: 'middle', size: 11.5, weight: 600, fill: DARK }); });
    const ly = legend(s, L1, B + 52, cats.map(x => [x[2], x[1]]));
  ...
  function f05(host, D) {
    ...
    function band(x1, a1, b1, x2, a2, b2, fill) {
      const mx = (x1 + x2) / 2;
      el('path', {
    ...
    cols.forEach((c, ci) => {
      c.items.forEach(([k, v]) => {
        const n = c.nodes[k];
        el('rect', { x: c.x, y: n.y0, width: NW, height: Math.max(2, n.y1 - n.y0), rx: 2, fill: ci === 1 ? (ICOL[k] || TEAL) : INK }, s);
  ...
  function f06(host, D) {
    ...
    d.types.forEach((t, i) => {
      ...
      sets.forEach(([k, col], j) => {
        ...
        el('rect', { x: L, y: by, width: Math.max(1, X(v) - L), height: bh, fill: col, rx: 2 }, s);
        txt(s, X(v) + 7, by + bh / 2 + 1, v + '%', { base: 'middle', size: 11, fill: GREY });
      });
    });
    const ly = legend(s, L, top + d.types.length * rh + 24, sets.map(x => [x[1], x[0]]));
    finish(s, W, footer(s, W, ly, d.source, pad));
  }
  ...
  function f07(host, D) {
    ...
    d.types.forEach((t, i) => {
      ...
      d.stages.forEach((st, j) => {
        ...
        el('rect', { x: L + j * cw + 2, y: y + 2, width: cw - 4, height: rh - 6, rx: 3, fill: fill }, s);
  ...
  function f08(host, D) {
    ...
    d.years.forEach((yr, i) => {
      ...
      keys.forEach(([k, col]) => {
        const v = d[k][i];
        if (v <= 0) return;
        el('rect', { x: x, y: Y(acc + v), width: bw, height: Y(acc) - Y(acc + v), fill: col }, s);
    ...
    const ly = legend(s, L, B + 52, keys.map(k => [k[1], k[0]]));
  ...
  function f09(host, D) {
    ...
    rows.forEach(([label, a, b, ta, tb], i) => {
      ...
      el('circle', { cx: X(a), cy: y, r: 7.5, fill: MID }, s);
      el('circle', { cx: X(b), cy: y, r: 7.5, fill: b >= a ? TEAL : LIME }, s);
    ...
    regs.forEach((r, i) => {
      ...
      el('rect', { x: pl, y: y, width: w, height: 16, rx: 2, fill: i === 0 ? TEAL : PALE }, s);
  ...
  function f10(host, D) {
    ...
    d.funds.forEach((f, i) => {
      ...
      keys.forEach(([k, col]) => {
        ...
        if (!v) return;
        const w = v / 100 * (R - L);
        el('rect', { x: x, y: y, width: w, height: bh, fill: col }, s);
        if (w > 38) txt(s, x + w / 2, y + bh / 2, v + '%', { anchor: 'middle', base: 'middle', size: 12, weight: 600, fill: ink(col) });
        x += w;
      });
    });
    const ly = legend(s, L, top + d.funds.length * (bh + gap) + 10, keys.map(k => [k[1], k[0]]), { size: 12 });
    finish(s, W, footer(s, W, ly, d.source, pad));
  }
  ...
  function f11(host, D) {
    ...
    a.years.forEach((yr, i) => {
      ...
      if (v > 0) {
        el('rect', { x: x, y: Y(v), width: bw, height: B - Y(v), fill: TEAL, rx: 2 }, s);
    ...
    rows.forEach((r, i) => {
      ...
      el('rect', { x: RL, y: y + 2, width: Math.max(1.5, w), height: 15, rx: 2, fill: ramp(0.35 + r[1] / rmax * 0.55) }, s);
  ...
  function f12(host, D) {
    ...
    rects.forEach(r => {
      ...
      el('rect', { x: r.x + 1.5, y: r.y + 1.5, width: Math.max(0, r.w - 3), height: Math.max(0, r.h - 3), rx: 3, fill: fill }, s);
  ...
  function f15(host, D) {
    ...
    rows.forEach(([label, v, kind], i) => {
      ...
      el('rect', { x: L, y: y + 4, width: Math.max(2, X(v) - L), height: 15, rx: 2, fill: col }, s);
      txt(s, X(v) + 8, y + 11, nice(v), { base: 'middle', size: 11, weight: 600, fill: kind === 'fund' ? '#6E8F16' : TEAL });
    });
    const ly = legend(s, L, top + rows.length * rh + 26, [[LIME, 'Dedicated climate funds'], [TEAL, 'MDB adaptation projects (WRI AdapTDR)']]);
  ...
  function f18(host, D) {
    ...
    keys.forEach((k, i) => {
      ...
      const w = v / mx * (R - L);
      el('rect', { x: L, y: y, width: Math.max(2, w), height: 32, rx: 3, fill: ramp(0.3 + v / mx * 0.6) }, s);
    ...
    t.forEach((r, i) => {
      ...
      el('rect', { x: TL, y: y, width: w1, height: 15, rx: 2, fill: TEAL }, s);
      if (w2) el('rect', { x: TL + w1, y: y, width: w2, height: 15, rx: 2, fill: LIME }, s);
    ...
    ly = legend(s, TL - 250, ly, [[TEAL, 'To approval'], [LIME, 'Approval to first disbursement']], { size: 11.5 });
  ...
  function f19(host, D) {
    ...
    rows.forEach((r, i) => {
      ...
      el('rect', { x: L, y: y + 4, width: Math.max(2, X(r.lev) - L), height: 21, rx: 2, fill: ramp(0.3 + r.lev / mx * 0.6) }, s);
  ...
  function f20(host, D) {
    ...
    items.forEach(([k, v, col], i) => {
      ...
      const w = v / mx * (R - L);
      if (v > 0) el('rect', { x: L, y: y, width: w, height: 30, rx: 3, fill: col }, s);
      else el('rect', { x: L, y: y + 12, width: 26, height: 5, rx: 2.5, fill: '#C9C4BE' }, s);
    ...
    dd.forEach((r, i) => {
      ...
      el('rect', { x: TL, y: y + 2, width: Math.max(2, w), height: 15, rx: 2, fill: ramp(0.35 + r[1] / tmax * 0.55) }, s);
  ...
  function f21(host, D) {
    ...
    rows.forEach(([name, a, b, kind], i) => {
      ...
      el('rect', { x: x1, y: y + 4, width: Math.max(3, x2 - x1), height: 15, rx: 3, fill: col }, s);
    ...
    ly = legend(s, L - 300, ly, Object.keys(KIND).slice(0, 5).map(k => [KIND[k], k]), { size: 11.5 });
    ly = legend(s, L - 300, ly + 4, Object.keys(KIND).slice(5).map(k => [KIND[k], k]), { size: 11.5 });
  ...
  function f22(host, D) {
    ...
    rows.forEach(item => {
      ...
      if (hi > lo) {
        el('rect', { x: X(lo), y: y + 5, width: X(hi) - X(lo), height: 13, rx: 6.5, fill: col }, s);
        txt(s, X(hi) + 9, y + 11, `${lo} to ${hi}`, { base: 'middle', size: 10.5, fill: GREY });
      } else {
        el('circle', { cx: X(lo), cy: y + 11.5, r: 6.5, fill: col }, s);
    ...
    const ly = legend(s, L - 300, y + 20, [[LIME, 'Ex ante appraisal'], [TEAL, 'Ex post evaluation'], ['#6E9E94', 'Modelled']]);
  ...
  function f23(host, D) {
    ...
    names.forEach((nm, i) => {
      ...
      row.forEach((v, j) => {
        if (!v) return;
        const w = v / 100 * (R - L);
        el('rect', { x: x, y: y, width: w, height: bh, fill: parts[j][1] }, s);
    ...
    const ly = legend(s, L, top + names.length * (bh + gap) + 12, parts.map(p => [p[1], p[0]]), { size: 12 });
    finish(s, W, footer(s, W, ly, d.source, pad));
  }
  ...
  function f27(host, D) {
    ...
    d.rows.forEach((r, i) => {
      ...
      d.cols.forEach((c, j) => {
        ...
        el('rect', { x: L + j * cw + 2, y: y + 2, width: cw - 4, height: rh - 5, rx: 3, fill: fill }, s);
  ...
  function f28(host, D) {
    ...
    sc.sectors.forEach((sec, i) => {
      ...
      if (u) {
        el('rect', { x: cx - 60 - wu, y: y + 22, width: wu, height: 15, rx: 2, fill: TEAL }, s);
        txt(s, cx - 66 - wu, y + 29, nice(u), { anchor: 'end', base: 'middle', size: 10.5, fill: GREY });
      }
      if (r) {
        el('rect', { x: cx + 60, y: y + 22, width: wr, height: 15, rx: 2, fill: LIME }, s);
    ...
    gitems.forEach(([k, v, col], i) => {
      ...
      el('rect', { x: GL, y: y + 22, width: v / 50 * (GR - GL), height: 18, rx: 2, fill: col }, s);
  ...
  function f29(host, D) {
    ...
    rows.forEach(([k, v], i) => {
      ...
      el('rect', { x: L, y: y + 4, width: Math.max(2, X(v) - L), height: 21, rx: 2, fill: hit ? LIME : ramp(0.3 + v / mx * 0.5) }, s);
  ...
  function f35(host, D) {
    ...
    rows.slice().sort((a, b) => b.population_2023 - a.population_2023).forEach(r => {
      ...
      el('circle', {
        ...
        r: rr, fill: col, opacity: 0.62, stroke: '#fff', 'stroke-width': 1.2
      }, s);
    });
    ...
    txt(s, 16, (top + B) / 2, 'USD per person, log scale', { anchor: 'middle', size: 12.5, fill: GREY })
      .setAttribute('transform', `rotate(-90 16 ${(top + B) / 2})`);
    legend(s, R + 40, top + 10, Object.keys(REGCOL).map(k => [REGCOL[k], k + ' Africa']), { column: true, size: 12 });
  ...
  function drawAfrica(s, P, fillFor) {
    const G = window.AFRICA || {};
    Object.keys(G).forEach(iso => {
      const key = ISO_ALIAS[iso] || iso;
      const f = fillFor(key) || '#F0EEEB';
      el('path', { d: geoPath(G[iso], P), fill: f, stroke: '#fff', 'stroke-width': 0.8, 'stroke-linejoin': 'round' }, s);
    });
  }
  function regionOf(D) {
    const m = {};
  ...
  function f33(host, D) {
    ...
    const drawn = names.slice().sort((a, b) => d[b][2] - d[a][2]).map(k => {
      ...
      el('circle', { cx: p[0], cy: p[1], r: r, fill: v > 0 ? TEAL : 'none', opacity: v > 0 ? 0.62 : 1, stroke: v > 0 ? '#fff' : LIME, 'stroke-width': v > 0 ? 1.4 : 2.2, 'stroke-dasharray': v > 0 ? null : '3 3' }, s);
